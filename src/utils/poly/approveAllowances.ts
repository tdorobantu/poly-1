...
const approveAllowances = async (
  ...
  console.log(
    `💵 USDC Allowance for CTF: ${ethers.utils.formatUnits(
      usdcAllowanceCtf,
      6
    )}`
  );
  console.log(
    `💵 USDC Allowance for Exchange: ${ethers.utils.formatUnits(
      usdcAllowanceExchange,
      6
    )}`
  );
  console.log(
    `🎲 Conditional Tokens Approval for Exchange: ${conditionalTokensAllowanceExchange}`
  );
  ...
  if (usdcAllowanceExchange.lte(constants.Zero)) {
    ...
    console.log(
      `✅ USDC allowance for Exchange set. Transaction Hash: ${txn.hash}`
    );
  }
  ...
  if (!conditionalTokensAllowanceExchange) {
    ...
    console.log(
      `✅ Conditional Tokens approval for Exchange set. Transaction Hash: ${txn.hash}`
    );
  }
